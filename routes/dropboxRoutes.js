...
router.post('/connect', async (req, res) => {
    ...
    try {
        ...
        console.log('Received request body:', { userId, accessToken: accessToken ? 'present' : 'missing', refreshToken: refreshToken ? 'present' : 'missing', expiresIn });
        ...
        const userInfo = await axios.post('https://api.dropboxapi.com/2/users/get_current_account', null, {
            headers: {
        ...
        user.dropboxId = userInfo.data.account_id;
        user.dropboxAccessToken = accessToken;
        user.dropboxRefreshToken = refreshToken;
        user.dropboxTokenExpiry = new Date(Date.now() + expiresIn * 1000);
...
router.post('/files', async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(userId) || !mongoose.Types.ObjectId.isValid(projectId)) {
            return res.status(400).json({ error: "Invalid userId or projectId format" });
        }
    ...
    try {
        ...
        for (const change of list_folder.entries) {
            ...
            if (new Date() >= user.dropboxTokenExpiry) {
                ...
                user.dropboxTokenExpiry = new Date(Date.now() + tokenResponse.data.expires_in * 1000);
                await user.save();
            }

            // Get file changes from Dropbox
            const changes = await axios.post('https://api.dropboxapi.com/2/files/get_metadata', {
                path: path_display,
...
router.post('/paper', async (req, res) => {
    try {
        ...
        const paperInfo = await axios.get(`https://api.dropboxapi.com/2/paper/docs/get_metadata`, {
            headers: {
                'Authorization': `Bearer ${user.dropboxAccessToken}`,
                'Content-Type': 'application/json'
            },
...
router.get('/paper/:paperDocId/changes', async (req, res) => {
    try {
        ...
        const comments = await axios.get(`https://api.dropboxapi.com/2/paper/docs/comments/list`, {
            headers: {
                'Authorization': `Bearer ${user.dropboxAccessToken}`,
                'Content-Type': 'application/json'
            },
        ...
        const suggestions = await axios.get(`https://api.dropboxapi.com/2/paper/docs/suggestions/list`, {
            headers: {
                'Authorization': `Bearer ${user.dropboxAccessToken}`,
                'Content-Type': 'application/json'
            },
